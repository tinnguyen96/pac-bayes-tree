...
if (dataset_name == "diagnosis"):
    ...
    # sample_size = np.linspace((total_size-test_size)/2,total_size-test_size,num=5): again the prunings and the aggregate have near identical classification error but different ROC curves 
...
if (train_index == 4):
    # log ratio margin 
    _, misClassifiedVector, margin_list = classifier.margin_distribution(X_test,y_test,None,"log ratio")
    ...
    # average gap margin 
    _, misClassifiedVector, margin_list = classifier.margin_distribution(X_test,y_test,None,"regression value")
    margin_list = np.asarray(margin_list,dtype=float)

    filename = 'MD/each experiment, each train size/' + dataset_name + '-' + classifer_name + "-margin=regression value" +  '-ex' + str(ex_index)+'tr'+str(train_index)+'.npy'
    margins = np.concatenate((misClassifiedVector.reshape(test_size,1), margin_list.reshape(test_size,1)), axis=1)
    np.save(filename,margins)